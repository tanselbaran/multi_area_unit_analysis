...
%each cluster
cluster_sites_full = zeros(size(meanWfGlobal,3), size(meanWfGlobal,2));
for i=1:size(meanWfGlobal,3)
   cluster_sites_full(i,:) = ismember(meanWfGlobal(1,:,i), meanWfLocal(1,:,i));
end
...
sortedSpikes = find(ismember(spikeClusters,singleUnitClusters));
spikeTimesSorted = spikeTimes(sortedSpikes);
sortedSpikeClusters = spikeClusters(sortedSpikes);
...
%JRClust
spikesFiltFull = zeros(size(meanWfGlobal,1),size(meanWfGlobal,2),size(sortedSpikes,1),'int16');
